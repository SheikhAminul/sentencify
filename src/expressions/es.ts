...
const intensifiers = `muy|bastante|realmente|súper|super|tan|demasiado|extremadamente|increíblemente|absolutamente|totalmente|completamente|verdaderamente|sumamente|tremendamente|terriblemente|profundamente|francamente|infinitamente|de verdad|auténtic[oa]|complet[oa]`
 ...
 */
const stateVerbs = `(?:es|fue|fueron|será[n]?|estaba[n]?|está[n]?|son|están|parece[n]?|parecía[n]?|queda[n]?|quedó|(?:ha|han)\\s+quedado|(?:ha|han)\\s+sido|anda[n]?|resulta[n]?|resultó|sale[n]?|salió|salieron|llega[n]?|llegó|llegaron|pasa[n]?|pasó|acaba[n]?|acabó|acabaron|se\\s+puso|se\\s+pone[n]?|trabaja[n]?|tienes?|tiene[n]?|corre[n]?|brilla[n]?|ruge[n]?|huele[n]?|duele[n]?|cruje[n]?|mueve[n]?|nota[n]?|desliza[n]?)`
...
const exclamAdj = `incre[íi]bles?|genial(?:es)?|fantástic[oa]s?|maravillos[oa]s?|espectacular(?:es)?|impresionantes?|alucinantes?|flipantes?|brutal(?:es)?|tremend[oa]s?|bestial(?:es)?|magnífic[oa]s?|estupend[oa]s?|excelentes?|perfect[oa]s?|precios[oa]s?|hermos[oa]s?|divin[oa]s?|adorables?|encantador(?:a|es|as)?|entrañables?|conmovedor(?:a|es|as)?|emocionantes?|inolvidables?|únic[oa]s?|mágic[oa]s?|delicios[oa]s?|exquisit[oa]s?|horribles?|terribles?|espantos[oa]s?|asqueros[oa]s?|repugnantes?|desastros[oa]s?|catastrófic[oa]s?|vergonzos[oa]s?|escandalos[oa]s?|inadmisibles?|inaceptables?|indignantes?|injust[oa]s?|ridícul[oa]s?|absurd[oa]s?|penos[oa]s?|lamentables?|fatal(?:es)?|guay|chul[oa]s?|tont[oa]s?|idiotas?|imbéciles?|estúpid[oa]s?|cretin[oa]s?|cautivador(?:a|es|as)?|fascinantes?|persuasiv[oa]s?|pésim[oa]s?|insoportables?|ensordecedor(?:a|es|as)?|superior(?:es)?|sobresalientes?|positiv[oa]s?|riguros[oa]s?|nítid[oa]s?|enorme[s]?|agotador(?:a|es|as)?|ideal(?:es)?|estrict[oa]s?|concis[oa]s?|interminables?|fuera\\s+de\\s+rango`

/** Nouns that make "es un/una N" a reaction: "es un desastre", "eres un crack". */
const exclamNoun = `escándalo|desastre|horror|asco|pasada|pasote|maravilla|locura|barbaridad|primor|encanto|crack|genio|máquina|fenómeno|fiera|bestia|sueño|regalo|milagro|lujo|espanto|robo|timo|abuso|vergüenza|chollo|pesadilla|infierno|gloria|joya|tesoro|monstruo|obra\\s+de\\s+arte|pena|lástima|drama|tragedia|caos|lío|desmadre|despropósito|disparate|atrocidad|pasada\\s+total|desastre\\s+total|caos\\s+total|notición|éxito|fortuna|eternidad|dolor\\s+de\\s+cabeza|dormilón|genialidad|basura`
...
export const es: SentenceTypeDetectExpressionSets = [
	...
	},
	{
		...
		expression: new RegExp(`^${lead}(?:(?:${DET}|eso|esto|aquello|todo|hoy|ayer|mañana)\\s+)?(?:${WORD}\\s+){0,6}(?:eres|sois|es|son|estás|estáis|está[n]?|estaba[n]?|fue|fueron|será[n]?|era[n]?|parece[n]?|resultó|quedó|salió|luce[n]?|lució|costó|tardó|(?:ha|han)\\s+sido|(?:ha|han)\\s+quedado)\\s+(?:(?:${intensifiers})\\s+)?(?:un[ao]?\\s+(?:${exclamNoun})|(?:${exclamAdj}))${WB}`, 'i'),
		type: 'exclamatory'
	},
	{
	...
	},
	{
		...
		expression: new RegExp(`^${lead}te\\s+(?:gusta[n]?|gustó|gustaría[n]?|apetece[n]?|parece[n]?|pareció|interesa[n]?|importa[n]?|molesta[n]?|duele[n]?|suena[n]?|toca|queda[n]?|falta[n]?|sobra[n]?|conviene[n]?|pasa|pasó|agradó|convenció|satisfizo|sorprendió|causó|(?:llama|llamó)\\s+la\\s+atención|hace\\s+falta|acuerdas|animas|atreves|enteras|imaginas|${WORD}(?:as|es|ás|és|áis|éis|aste|iste))${WB}`, 'i'),
		type: 'interrogative'
	},
	{
	...
	},
	{
		...
		expression: new RegExp(`^${lead}qué\\s+(?!${pron}${WB})(?!${verbForms}${WB})(?!${questionNoun}${WB})${WORD}(?:\\s+${WORD}){0,2}\\s+(?:${pron}\\s+){0,2}(?:${stateVerbs}|${WORD}(?:ó|amos|emos|imos|aste|iste))\\s+(?:${DET}\\s+${WORD}(?:\\s+${WORD}){0,3}|${WORD}(?:ar|er|ir|arse|erse|irse)(?:\\s+${WORD}){0,3})$`, 'i'),
		type: 'exclamatory'
	},
	{
	...
	},
	{
		...
		expression: new RegExp(`\\b(?:el|la|los|las|lo)\\s+(?:mejor|peor|más\\s+${WORD})${WB}[^,]{0,40}(?:que\\s+(?:he|hemos|has|han)\\s+${WORD}|de\\s+(?:mi\\s+vida|la\\s+historia|todos\\s+los\\s+tiempos|el\\s+año|el\\s+mundo))${WB}`, 'i'),
		type: 'exclamatory'
	},
	{
	...
	},
	{
		...
		expression: new RegExp(`\\b(?:de\\s+maravilla|estupendamente|de\\s+forma\\s+extraordinaria|por\\s+completo|para\\s+nada|absolutamente\\s+nada|ni\\s+un[oa]?\\s+${WORD}|por\\s+ningún\\s+lado|por\\s+más\\s+que|sin\\s+palabras)${WB}`, 'i'),
		type: 'exclamatory'
	},
