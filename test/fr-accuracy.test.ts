...
const fixturesPath = join(dirname(fileURLToPath(import.meta.url)), '../../test/fr.json')
const fixtures = JSON.parse(readFileSync(fixturesPath, 'utf8')) as Fixture[]

/** Minimum accuracy the current fr.ts rule set must keep clearing, so a rule-ordering regression fails loudly instead of silently. */
const minAccuracyPerType = 90
const minOverallAccuracy = 95

describe('detectSentenceType — accuracy against test/fr.json', () => {
	it('classifies real-world French sentences and reports accuracy by type', () => {
		...
		for (const [sentence, expected] of fixtures) {
			...
			const detected = detectSentenceType(normalizedSentence, 'fr')
			const typeStats = statsByType.get(expected)
			assert.ok(typeStats, `Unknown sentence type "${expected}" in test/fr.json fixture for: ${sentence}`)
			typeStats.total++
		...
		console.log('\ndetectSentenceType accuracy — test/fr.json')
		console.table(table)
...
**RESEARCH**
Before generating the data, research all relevant scenarios, subtypes, exceptions, and special cases of French declarative, interrogative, and exclamatory sentences.
...
**DOMAIN**
Natural French sentences from realistic human conversations.
...
**DOMAIN**
Natural French messages people send to AI chatbots (coding, writing, debugging, learning, planning, reactions, casual chat, frustration, praise).
