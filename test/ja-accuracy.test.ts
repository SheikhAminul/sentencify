...
const fixturesPath = join(dirname(fileURLToPath(import.meta.url)), '../../test/ja.json')
const fixtures = JSON.parse(readFileSync(fixturesPath, 'utf8')) as Fixture[]

/** Minimum accuracy the current ja.ts rule set must keep clearing, so a rule-ordering regression fails loudly instead of silently. */
const minAccuracyPerType = 90
const minOverallAccuracy = 95

describe('detectSentenceType — accuracy against test/ja.json', () => {
	it('classifies real-world Japanese sentences and reports accuracy by type', () => {
		...
		for (const [sentence, expected] of fixtures) {
			...
			const detected = detectSentenceType(normalizedSentence, 'ja')
			const typeStats = statsByType.get(expected)
			assert.ok(typeStats, `Unknown sentence type "${expected}" in test/ja.json fixture for: ${sentence}`)
			typeStats.total++
		...
		console.log('\ndetectSentenceType accuracy — test/ja.json')
		console.table(table)
...
**RESEARCH**
Before generating the data, research all relevant scenarios, subtypes, exceptions, and special cases of Japanese declarative, interrogative, and exclamatory sentences.
...
**DOMAIN**
Natural Japanese sentences from realistic human conversations.
...
**DOMAIN**
Natural Japanese messages people send to AI chatbots (coding, writing, debugging, learning, planning, reactions, casual chat, frustration, praise).
