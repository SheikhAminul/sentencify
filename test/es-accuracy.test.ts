...
const fixturesPath = join(dirname(fileURLToPath(import.meta.url)), '../../test/es.json')
const fixtures = JSON.parse(readFileSync(fixturesPath, 'utf8')) as Fixture[]

/** Minimum accuracy the current es.ts rule set must keep clearing, so a rule-ordering regression fails loudly instead of silently. */
const minAccuracyPerType = 67.5
const minOverallAccuracy = 75

describe('detectSentenceType — accuracy against test/es.json', () => {
	it('classifies real-world Spanish sentences and reports accuracy by type', () => {
		...
		for (const [sentence, expected] of fixtures) {
			const normalizedSentence = sentence.toLowerCase().replace(/^[¿¡]|[.¿?¡!]$/g, '')
			const detected = detectSentenceType(normalizedSentence, 'es')
			const typeStats = statsByType.get(expected)
			assert.ok(typeStats, `Unknown sentence type "${expected}" in test/es.json fixture for: ${sentence}`)
			typeStats.total++
		...
		console.log('\ndetectSentenceType accuracy — test/es.json')
		console.table(table)
...
**RESEARCH**
Before generating the data, research all relevant scenarios, subtypes, exceptions, and special cases of Spanish declarative, interrogative, and exclamatory sentences.
...
**DOMAIN**
Natural Spanish sentences from realistic human conversations.
...
**DOMAIN**
Natural Spanish messages people send to AI chatbots (coding, writing, debugging, learning, planning, reactions, casual chat, frustration, praise).
