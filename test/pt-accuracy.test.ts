...
const fixturesPath = join(dirname(fileURLToPath(import.meta.url)), '../../test/pt.json')
const fixtures = JSON.parse(readFileSync(fixturesPath, 'utf8')) as Fixture[]

/** Minimum accuracy the current pt.ts rule set must keep clearing, so a rule-ordering regression fails loudly instead of silently. */
const minAccuracyPerType = 55
const minOverallAccuracy = 70

describe('detectSentenceType — accuracy against test/pt.json', () => {
	it('classifies real-world Portuguese sentences and reports accuracy by type', () => {
		...
		for (const [sentence, expected] of fixtures) {
			...
			const detected = detectSentenceType(normalizedSentence, 'pt')
			const typeStats = statsByType.get(expected)
			assert.ok(typeStats, `Unknown sentence type "${expected}" in test/pt.json fixture for: ${sentence}`)
			typeStats.total++
		...
		console.log('\ndetectSentenceType accuracy — test/pt.json')
		console.table(table)
...
**RESEARCH**
Before generating the data, research all relevant scenarios, subtypes, exceptions, and special cases of Portuguese declarative, interrogative, and exclamatory sentences.
...
**DOMAIN**
Natural Portuguese sentences from realistic human conversations.
...
**DOMAIN**
Natural Portuguese messages people send to AI chatbots (coding, writing, debugging, learning, planning, reactions, casual chat, frustration, praise).
